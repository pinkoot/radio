...
% Параметры
f_growth = 2 / (tau_imp * f_diskr);        % Коэффициент роста частоты
N = length(time_of_modeling);              % Кол-во отсчётов
dt = 1 / f_diskr;                          % Период дискретизации
L = N;                                     % Нормировка спектра

% Формирование мгновенной частоты
freq = zeros(1, N);
for i = 1:N
    if i <= tau_imp * f_diskr
        freq(i) = i * f_growth;
    else
        freq(i) = tau_imp * f_diskr * f_growth;  % Постоянная после импульса
    end
end

% Интеграл частоты -> фаза
phase = 2*pi * cumsum(freq) * dt;

% ЛЧМ сигнал
Lchm_sign = sin(phase);

% FFT
fft_LCHM = fft(Lchm_sign, n_count_for_fft);
fft_LCHM = fft_LCHM(1:n_count_for_fft/2);

% Амплитуда и фаза спектра
Real_fft_LCHM = real(fft_LCHM);
Imagine_fft_LCHM = imag(fft_LCHM);
Ampl_fft_LCHM = abs(fft_LCHM) / L;  
Phase_LCHM = atan2(Imagine_fft_LCHM, Real_fft_LCHM);

% Частотная шкала
Frequencies = (0:(n_count_for_fft/2 - 1)) * (f_diskr / n_count_for_fft);
...
xlabel('Частота, Гц'); ylabel('Фаза');
title('Фазовый спектр ЛЧМ сигнала');
axis([0 max(Frequencies) -pi pi]);
...
axis([0 2*f_car -pi-0.5 pi+0.5]);
%% ====== Фазовый спектр М-последовательности (исправленный) ======
